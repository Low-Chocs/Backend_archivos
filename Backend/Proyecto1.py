...
def new_start(instruction: str):   
    print(api.global_text)
    api.global_text = ""
    print("afkjdafsj")
    if instruction == "":
        print(f"Línea en blanco ")
        return
    index = instruction.find("#")
    if index != -1:
        text_after_hash = instruction[index + 1:].strip()
        api.global_text += f"Comentario: {text_after_hash} (Línea)"
        print(f"Comentario: {text_after_hash} (Línea)")
        return
    instructions_splitted = instruction.split()
    print(instructions_splitted)
    check_next_instruction(instructions_splitted)
    print("Este es el texto de prueba total:")
    print(api.global_text)
    return api.global_text

def verificar_carpeta(ruta):
    carpeta_objetivo = 'tmp'
    if os.path.exists(ruta) and os.path.isdir(ruta):
        contenido = os.listdir(ruta)
        if carpeta_objetivo in contenido and os.path.isdir(os.path.join(ruta, carpeta_objetivo)):
            print(f"La carpeta '{carpeta_objetivo}' existe en la ruta {ruta}.")
        else:
            nueva_ruta = os.path.join(ruta, carpeta_objetivo)
            os.mkdir(nueva_ruta)
            print(f"La carpeta '{carpeta_objetivo}' ha sido creada en la ruta {ruta}.")
    else:
    ...
    try:
        with open(path, 'r') as search_file:
            ...
            for instruction in search_file:
                ...
                if index != -1:
                    text_after_hash = instruction[index + 1:].strip()
                    api.global_text += f"Comentario: {text_after_hash} (Línea {line_number})"
...
def rep(instruction):
    ...
    for i in range(len(instruction)):
        ...
        if instruction_set[0].lower() == 'name':
            ...
            api.global_text += f"Error no se reconocio el comando {instruction_set[0]} \n"
            return
    names = get_file_name(path)
    reports(name, names, id, route)
...
def new_disk(size: int, path: str, fit: str, unit: str):
    ...
    if unit.lower() == 'k':
        ...
        api.global_text += "Error el timpo debe ser k o m \n"
    ...
    if os.path.exists(path):
        api.global_text += "Ya existe un disco con el mismo nombre \n"
...
def delete_disk(path: str):
    try:
        os.remove(path)
        api.global_text += f"El archivo {path} ha sido eliminado correctamente. \n"
        print(f"El archivo {path} ha sido eliminado correctamente.")
    except FileNotFoundError:
        api.global_text += f"El archivo {path} no se encontró."
        print(f"El archivo {path} no se encontró.")
    except Exception as e:
        api.global_text += f"Se produjo un error al eliminar el archivo: {str(e)}"
...
def new_partition(size: str, path: str, name: str, unit: str, type: str, fit: str, delete: str, new_add: int):
    ...
    if unit.lower() == 'k':
        ...
        api.global_text += "El tipo debe ser 'k' o 'm'"
        raise ValueError("El tipo debe ser 'k' o 'm'")
    if delete != 0:
        api.global_text += f"Se procedera eliminar {path}, la particion {name}"
        print(f"Se procedera eliminar {path}, la particion {name}")
        mbr = MBR(0,0,0,0)
        mbr.delete_partition(path, name)
        return
    if new_add != 0:
        api.global_text += f"Se procedera a agregar {new_add} en la ruta: {path} con el nombre: {name}"
...
def print_mount():
    ...
    for i in mounted_list:
        count += 1
        api.global_text += f"Nombre del disco {i.name} Id del disco {i.id} Path {i.path}"
        print(f"Nombre del disco {i.name} Id del disco {i.id} Path {i.path}")
    if count == 0:
        api.global_text += "No hay particiones montadas"
...
def check_mounted(name: str):
    for i in mounted_list:
        if name.lower() == i.id.lower():
            ...
            print(f"Nombre del disco {i.name} Id del disco {i.id} Path {i.path}")
            return i
    api.global_text += f"ERROR: No se encontro ese mount con el id: {name}"     
...
def unmounted(name: str):
    for i in range(len(mounted_list)):
        if name.lower() == mounted_list[i].id.lower():
            api.global_text += f"Se desmontara el disco {mounted_list[i].name} Id del disco {mounted_list[i].id} Path {mounted_list[i].path}" 
